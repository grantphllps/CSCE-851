...
/*
The original code provided was used as a starting point and later modified by Grant Phillips
 */
        ...
        try {
            ...
            for (const auto& cmd : shell_commands) {
                ...
                if (wstatus == 0 || alwaysExecuteNext) { //If the exit status is 0, execute the next command in shell commands
                    ...
                    //The following if-else are used to control the closing of pipes depending if the pipe is at the beginning, middle, or end of a chain
                    if (cmd.cout_mode == ostream_mode::pipe && cmd.cin_mode != istream_mode::pipe) { //First pipe in a chain
                        pipeF = true;
                        pipeM = false;
                        pipeL = false;
                        piping = true;
                    ...
                    else if (cmd.cout_mode != ostream_mode::pipe && cmd.cin_mode == istream_mode::pipe) {//Last pipe in a chain
                        pipeF = false;
                        pipeM = false;
                        pipeL = true;
                        piping = true;
                    }
                    else if (cmd.cout_mode == ostream_mode::pipe && cmd.cin_mode == istream_mode::pipe) {//Middle pipe in a chain
                        pipeF = false;
                        pipeM = true;
                        pipeL = false;
                        piping = true;
