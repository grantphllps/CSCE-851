...
void *start_worker(void *arg)
{
	...
	sigaddset(&signal_block_set, SIGUSR2);
	sigaddset(&signal_block_set, SIGALRM);
	pthread_sigmask(SIG_BLOCK, &signal_block_set,NULL);
	/* TODO: Unblock SIGUSR1 and SIGTERM. */
	sigset_t signal_unblock_set;
	sigemptyset (&signal_unblock_set);
	sigaddset(&signal_unblock_set, SIGUSR1);
	sigaddset(&signal_unblock_set, SIGTERM);
	pthread_sigmask(SIG_UNBLOCK, &signal_unblock_set,NULL);
