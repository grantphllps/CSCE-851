        ...
        try {
            ...
            for (const auto& cmd : shell_commands) {
                ...
                if (wstatus == 0 || alwaysExecuteNext) { //If the exit status is 0, execute the next command in shell commands
                    ...
                    //This was more relevent for early debugging, mainly to preserve the original cmd structure
                    ...
                    if (pid < 0) {
                        ...
                        // After the child process has started, need to take care of the input/output redirecting, if needed
                        int fd;
                        // output redirecting
