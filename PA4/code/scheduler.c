...
void update_run_time(thread_info_t *info) {
		...
		timespec_get(&(info->suspend_time), TIME_UTC);
...
void update_wait_time(thread_info_t *info) {
        /* TODO: implement this function */
		timespec_get(&(info->resume_time), TIME_UTC);

		//long temp1 = info->resume_time.tv_sec * 1000000 + info->resume_time.tv_nsec / 1000;
		//long temp2 = info->suspend_time.tv_sec * 1000000 + info->suspend_time.tv_nsec / 1000;

		// printf("The suspend time is: %ld\n",temp2);
		// printf("The resume time is: %ld\n",temp1);

		info->wait_time = time_difference(&info->resume_time,&info->suspend_time);
		//printf("The wait time is: %ld\n",info->wait_time);
}
